...
def load_target_paper_meta_info(
        config: Config,
        filename: str ="additional_ancestry_target_paper_list"
) -> pd.DataFrame:
...
def main() -> None:
    ...
    df_forward_search_result.to_csv(config.processed_data_dir / "additional_ancestry_search_result.csv", index=False)
